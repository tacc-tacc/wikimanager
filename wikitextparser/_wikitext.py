...
# Sections
SECTION_START = rb'^(={1,6})[^\n]+?(={1,6})[ \t]*+$'
SECTION_HEADING = rb'^(?<equals>={1,6})[^\n]+?(?P=equals)[ \t]*+$'
SUBSECTION_HEADING = rb'^(?P=equals)=[^\n]+?(?P=equals)=[ \t]*+$'
LEAD_SECTION = rb'(?<section>(?<equals>).*?)'

SECTION_START_FINDITER = rc(SECTION_START, DOTALL | MULTILINE | VERBOSE).finditer
...
class WikiText:
    ...
    def _section_title_spans_to_section_titles(
        self, section_spans: List[Tuple[int, int]], shadow: bytearray
    ) -> List['SectionTitle']:
        type_to_spans = self._type_to_spans
        ...
        sections_append = sections.append
        ss, se, _, ba = self._span_data
        type_spans = type_to_spans.setdefault('SectionTitle', [])
        span_tuple_to_span = {(s[0], s[1]): s for s in type_spans}.get
        lststr = self._lststr
        for ms, me in section_spans:
            s, e = ss + ms, ss + me
            ...
            if old_span is None:
                ...
                span = old_span
            sections_append(SectionTitle(lststr, type_to_spans, span, 'SectionTitle'))
        return sections
    ...
    def get_section_titles(
        ...
        *args
        ...
        shadow = self._shadow
        titles = []
        for m in SECTION_START_FINDITER(shadow):
            ms, me = m.span()
            titles.append((ms, me))

        return self._section_title_spans_to_section_titles(titles, shadow)
...
if __name__ == '__main__':
    ...
    from ._section_title import SectionTitle
